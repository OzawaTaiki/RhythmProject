...
void oldLane::Draw(const Camera* _camera)
{
...
Vector3 oldLane::GetLaneStartPosition(uint32_t _index) const
{
    if (!isDirty_)
...
void oldLane::AddNote(uint32_t _index, std::shared_ptr<Note> _note)
{
    if (_index >= laneStartPoints_.size())
    {
        throw std::out_of_range("Indexが配列サイズより大きいです。");
...
void oldLane::InitializeJsonBinder()
{
...
std::vector<Vector3> oldLane::GeneLaneStartPoints() const
{
    if(isDirty_)
