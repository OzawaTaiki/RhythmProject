...
void Lane::Initialize(const std::list<NoteData>& _noteDataList, int32_t _laneIndex, float _judgeLine)
{
    // レーンの座標を計算
    // 基準は(0,0,_judgeLine)
    ...
    startPosition_.z = laneTopEdge; // レーンの開始位置は指定された判定ライン

    endPosition_ = startPosition_;
    endPosition_.z -= laneLength_; // レーンの終了位置は開始位置からレーンの長さだけ手前

    // ノーツを生成
    CreateNotes(_noteDataList, _laneIndex, _judgeLine);

}

void Lane::Update(float _elapseTime)
{
...
void Lane::CreateNotes(const std::list<NoteData>& _noteDataList, int32_t _laneIndex, float _judgeLine)
{
    Vector3 noteTargetPosition = startPosition_;
    ...
    for (const auto& noteData : _noteDataList)
    {
        if (noteData.laneIndex == _laneIndex) // 念のためチェック
        {
            ...
            auto note = std::make_unique<NomalNote>();
            note->Initilize(startPosition_, noteData.targetTime, 0.0f, noteTargetPosition, _laneIndex);
